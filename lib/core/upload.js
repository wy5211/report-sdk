...
class UploadManager {
    constructor(config, adapter) {
        ...
        this.count = 0;
        this.count = 0;
        emitter_1.default.on(emitterKey_1.EmitterKeys.CACHE_LIMITED_REACHED, (data) => {
            logger_1.default.log('监听到上传缓存任务', data);
    ...
    run(data, cacheKey) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            let sendData = (0, request_1.requestInstance)({
                type: 'upload',
                body: data,
            ...
            let response;
            if (beforeSend && (0, utils_1.isFunction)(beforeSend)) {
                sendData = beforeSend(sendData);
            }
            try {
                logger_1.default.log('开始执行上报数据', sendData);
                response = (yield request(sendData));
                afterSend === null || afterSend === void 0 ? void 0 : afterSend(response);
                this.count = 0;
                // 清除缓存
                if (cacheKey) {
                    emitter_1.default.emit(emitterKey_1.EmitterKeys.CLEAR_CACHE, [cacheKey, data]);
                }
                clearTimeout(this.timer);
            }
            catch (error) {
                logger_1.default.log('执行上报数据出错', error, sendData);
    ...
    handleCacheUpload(event) {
        ...
        const sliceUpload = (eventType, sendData) => {
            (0, utils_1.chunk)(sendData, this.config.cacheUploadCount).forEach((item) => {
                this.run(Object.assign({ eventType, extInfo: item }, (JSON.parse(JSON.stringify(extraData)))), cacheKey);
            });
        };
        logger_1.default.log('开始分批上报数据', data);
